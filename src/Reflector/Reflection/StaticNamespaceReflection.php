    ...
    public function parseBody( $isBracketed, Tokenizer $t, AliasResolver $r  )
    {
        ...
        do {
            ...
            switch ($token[0]) {
                ...
                // interface
                case T_INTERFACE:
                    $interface = new StaticInterfaceReflection( $this, $this->factory, $t, $r );
                    $token = $t->getToken();
                ...
                case T_ABSTRACT:
                case T_CLASS:
                    $class = new StaticClassReflection( $this, $this->factory, $t, $r );
                    $token = $t->getToken();
    ...
    public function addItem( ReflectionInterface $item )
    {
        ...
        if ( isset( $this->items[ $item->getShortName() ] ) ) {
            ...
            if ($isReplacable === false) {
                throw new RedeclarationException( "Namespace {$this->getName()} already contains {$item->getShortName()}, previously declarladerd at {$previous->getFileName()}:{$previous->getStartLine()}" );
            }
        }
